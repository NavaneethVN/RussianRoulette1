...
playerOneShoots(){
    ...
    choice=$(dialog --title "$playeronename's turn" --menu "You have two
    choices" 10 30 2 Y "Shoot $playertwoname" N "Shoot yourself" \
                    3>&1 1>&2 2>&3 3>&- )
    case $choice in
        "Y")
            dialog --title "$playeronename's turn" --msgbox "You chose to shoot
$playertwoname" 10 40
            if [ "${revolver[$triggerplace]}" -eq 1 ]
            then
                ((playertwolives--))
                dialog --title "$playeronename's turn" --msgbox "You Killed
$playertwoname" 10 40
            else
                dialog --title "$playeronename's turn" --msgbox "$playertwoname survived "
10 40
            fi
            ((turn++))
            ((triggerplace++))
            ...
            ;;
        "N")
            dialog --title "$playeronename's turn" --msgbox "How generous of you to shoot
yourself instead of  $playertwoname" 30 40
            if [ "${revolver[$triggerplace]}" -eq 1 ]
            then
                ((playeronelives--))
                dialog --title "$playeronename's turn" --msgbox "oh oh Seems like Bad luck"
30 40
                ((turn++))
            else
                dialog --title "$playeronename's turn" --msgbox "You Survived,The
generosity paid off eh\nSince you shot Yourself and survived , as a
bonus you get to go again" 30 40
                ((turn++))
                ((turn++))
            fi
            ((countbullets++))
...
playerTwoShoots(){
    ...
    choice=$(dialog --title "$playertwoname's turn" --menu "You have two choices" 10
          30 2 Y \ "Shoot $playeronename" N "Shoot yourself" \ 3>&1 1>&2 2>&3 3>&- )
    case $choice in
        "Y")
            ...
            ;;
        "N")
            dialog --title "$playertwoname's turn" --msgbox "How generous of you to shoot
yourself \ instead of  $playeronename" 30 50
            if [ "${revolver[$triggerplace]}" -eq 1 ]
            then
                ((playertwolives--))
                dialog --title "$playertwoname's turn" --msgbox "oh oh Seems like Bad
luck\n$playertwoname Shot himself" 30 75
                ((turn++))
            else
                dialog --title "$playertwoname's turn" --msgbox "You Survived,The
generosity paid off eh\
